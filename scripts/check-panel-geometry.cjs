...
async function main() {
  ...
  for (const f of files) {
    ...
    // The lead-in (561f544) covers the stage and hides the footer bar, so
    // #btnNext has a display:none ancestor until it is dismissed. Without
    // this the first click throws and the run dies before measuring anything.
    // #skip-intro would also work, but clicking the real button exercises the
    // same path a reader takes.
  ...
  console.log(
    failures === 0
      ? `No overflowing panels across ${checked} file(s).`
      : `${failures} failure(s).`
  );
